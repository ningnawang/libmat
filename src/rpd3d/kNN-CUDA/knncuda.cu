 ...
 * distances
 */
__global__ void compute_distances(const float* ref, const int ref_width,
                                  const int ref_pitch, const float* query,
                                  const int query_width, const int query_pitch,
                                  const int height, float* dist) {
...
// ninwang: squared Euclidean distance - ref_point weight
__global__ void dist_minus_weight(float* dist, const int dist_pitch,
                                  const float* weight, const int width,
                                  const int height) {
  // Column position
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (xIndex < width) {
    ...
    // // ninwang: debug
    // if (xIndex == 543) {
    //   printf("width:%d, height: %d column %d: \n", width, height, xIndex);
    //   for (int i = 0; i < height; ++i) {
    //     float curr_dist = p_dist[i * dist_pitch];
    //     float curr_weight = weight[i];
    //     printf("ref_site: %d, curr_dist: %f, curr_weight: %f, diff %f \n", i,
    //            curr_dist, curr_weight, curr_dist - curr_weight);
    //   }
    //   printf("\n");
    // }

    // Go through all points in column
    for (int i = 0; i < height; ++i) {
      float curr_dist = p_dist[i * dist_pitch];
      float curr_weight = weight[i];
      p_dist[i * dist_pitch] = curr_dist - curr_weight;
    }
  }
}

// ninwang: seems not working fine
__global__ void dist_minus_weight_2(float* dist, const int dist_pitch,
                                    const float* weight, const int width,
                                    const int height) {
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
  if (xIndex < width && yIndex < height) {
    dist[yIndex * dist_pitch + xIndex] =
        dist[yIndex * dist_pitch + xIndex] - weight[yIndex];
  }
}

/**
 * For each qeury point (i.e. each column) finds the k-th smallest distances
 ...
 */
__global__ void modified_insertion_sort(float* dist, const int dist_pitch,
                                        int* index, const int index_pitch,
  ...
  if (xIndex < width) {
    ...
    // if (width == 1 || width == 2)
    //   printf(
    //       "xIndex: %d, width:%d, height: %d, k: %d, p_index[0]: %d, "
    //       "p_index[1]: "
    //       "%d, p_index[2]: %d, p_index[3]: %d \n",
    //       xIndex, width, height, k, p_index[0], p_index[1 * index_pitch],
    //       p_index[2 * index_pitch], p_index[3 * index_pitch]);
  }
}
 ...
 */
__global__ void compute_sqrt(float* dist, const int width, const int pitch,
                             const int k) {
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
 ...
 */
__global__ void compute_squared_norm(const float* array, const int width,
 ...
 */
__global__ void add_reference_points_norm(float* array, const int width,
                                          const int pitch, const int height,
                                          const float* norm) {
 ...
 */
__global__ void add_query_points_norm_and_sqrt(float* array, const int width,
                                               const int pitch, const int k,
                                               const float* norm) {
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
  if (xIndex < width && yIndex < k)
...
// ref_dev: all spheres, weight = radius^2
// query_dev: all tet vertices
//
// Return:
// pdist_dev: power distance matrix, #spheres x #tet_vertices
bool power_dist_cuda_global_dev(const float* ref_dev, const int ref_nb,
                                ...
                                float* pdist_dev, const size_t pdist_pitch) {
  // printf(
  //     "ref_nb: %d, ref_pitch: %d, query_nb: %d, "
  //     "query_pitch: %d, k: %d, index_pitch: %d\n",
  //     ref_nb, ref_pitch, query_nb, query_pitch, k, index_pitch);

  // Check pitch values
  if (query_pitch != pdist_pitch) {
  ...
  compute_distances<<<grid0, block0>>>(ref_dev, ref_nb, ref_pitch, query_dev,
                                       query_nb, query_pitch, dim, pdist_dev);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    return false;
  }

  // Compute the weighted squared Euclidean distances
  dim3 block1(256, 1, 1);
  dim3 grid1(query_nb / 256, 1, 1);
  if (query_nb % 256 != 0) grid1.x += 1;
  dist_minus_weight<<<grid1, block1>>>(pdist_dev, pdist_pitch, ref_weight,
                                       query_nb, ref_nb);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    return false;
...
// size of ref_weight must equals to ref_nb
bool knn_weighted_cuda_global_dev(const float* ref_dev, const int ref_nb,
  ...
  err0 = cudaMallocPitch((void**)&dist_dev, &dist_pitch_in_bytes,
  ...
  dim3 grid1(query_nb / 256, 1, 1);
  if (query_nb % 256 != 0) grid1.x += 1;
  dist_minus_weight<<<grid1, block1>>>(dist_dev, dist_pitch, ref_weight,
                                       query_nb, ref_nb);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(dist_dev);
  ...
  // dim3 block1(16, 16, 1);
  // dim3 grid1(query_nb / 16, k / 16, 1);
  // if (query_nb % 16 != 0) grid1.x += 1;
  // if (k % 16 != 0) grid1.y += 1;
  // dist_minus_weight_2<<<grid1, block1>>>(dist_dev, dist_pitch, ref_weight,
  //                                        query_nb, ref_nb);
  // if (cudaGetLastError() != cudaSuccess) {
  //   printf("ERROR: Unable to execute kernel\n");
  //   cudaFree(dist_dev);
  ...
  dim3 block2(256, 1, 1);
  dim3 grid2(query_nb / 256, 1, 1);
  if (query_nb % 256 != 0) grid2.x += 1;
  modified_insertion_sort<<<grid2, block2>>>(
      dist_dev, dist_pitch, knn_index_dev, index_pitch, query_nb, ref_nb, k);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(dist_dev);
    return false;
...
bool knn_cuda_global_dev(const float* ref_dev, const int ref_nb,
  ...
  dim3 grid1(query_nb / 256, 1, 1);
  if (query_nb % 256 != 0) grid1.x += 1;
  modified_insertion_sort<<<grid1, block1>>>(
      dist_dev, dist_pitch, knn_index_dev, index_pitch, query_nb, ref_nb, k);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(dist_dev);
  ...
  // ninwang:
  // we don't need this, causes lots of "Invalid __global__ read"
  //
  ...
  // if (k % 16 != 0) grid2.y += 1;
  // printf(
  //     "block2: (%d, %d, %d), grid2: (%d, %d, %d), query_nb: %d, query_pitch:
  //     "
  //     "%d, k: %d, dist_pitch: %d\n",
  //     block2.x, block2.y, block2.z, grid2.x, grid2.y, grid2.z, query_nb,
  //     query_pitch, k, dist_pitch);
...
//               query_dev, query_pitch, ref_dev, ref_pitch, (float)0.0,
...
//   add_reference_points_norm<<<grid2, block2>>>(dist_dev, query_nb,
//   dist_pitch,
//                                                ref_nb, ref_norm_dev);
//   if (cudaGetLastError() != cudaSuccess) {
//     printf("ERROR: Unable to execute kernel\n");
...
//   // Add query norm and compute the square root of the of the k first
//   elements dim3 block3(16, 16, 1); dim3 grid3(query_nb / 16, k / 16, 1); if
//   (query_nb % 16 != 0) grid3.x += 1; if (k % 16 != 0) grid3.y += 1;
