 ...
 *
 * This file has a software implementation of ConvexCell, as well
 * as a function to create a ConvexCell from the dual of a vertex
...
#define vbw_assert(x)                                               \
  if (!(x)) {                                                       \
    std::cerr << "Assertion fail: " << #x << " " << __FILE__ << " " \
              << __LINE__ << std::endl;                             \
    abort();                                                        \
  }
#else
...
inline vec3 cross(vec3 v1, vec3 v2) {
  return make_vec3(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
                   v1.x * v2.y - v1.y * v2.x);
}
 ...
 */
inline double length(vec3 v) { return ::sqrt(squared_length(v)); }
...
/**
 * \brief A vector with four components, used to store
 *  plane equations.
 */
struct vec4 {
  double x;
 ...
 * \return the length of \p v.
 */
inline double length(vec4 v) { return ::sqrt(squared_length(v)); }
...
inline TriangleWithFlags make_triangle_with_flags(ushort i, ushort j, ushort k,
                                                  ushort f) {
...
class ConvexCell {
 public:
   ...
   * \param[in] keep_vglobal if set, global vertex indices
   *  are stored.
   */
   ...
   */
  void init_with_box(double xmin, double ymin, double zmin, double xmax,
   ...
   */
  index_t save(std::ostream& out, index_t v_offset = 1,
               double shrink = 0.0) const;
  ...
  vec3 triangle_point(ushort t) const {
    if (geometry_dirty_) {
      ...
      return make_vec3(result.x / result.w, result.y / result.w,
                       result.z / result.w);
    }
 ...
 protected:
   ...
   * \param[in] e local index of an edge of \p t (in 0..2).
  ...
  bool triangle_is_infinite(index_t t) const {
    ...
    vbw_assert(t < max_t());
    Triangle T = get_triangle(t);
    return (T.i == VERTEX_AT_INFINITY || T.j == VERTEX_AT_INFINITY ||
  ...
  index_t new_triangle(index_t i, index_t j, index_t k) {
    ...
    if (result == END_OF_LIST) {
      ...
      first_free_ =
          index_t(get_triangle_flags(first_free_) & ~ushort(CONFLICT_MASK));
    }
    vbw_assert(result < max_t());
    t_[result] = make_triangle_with_flags(ushort(i), ushort(j), ushort(k),
                                          ushort(first_valid_));
    set_vv2t(i, j, result);
    set_vv2t(j, k, result);
    set_vv2t(k, i, result);
   ...
   */
  index_t new_triangle(index_t i, index_t j, index_t k, index_t adj0,
                       index_t adj1, index_t adj2) {
    geo_argused(adj0);
    geo_argused(adj1);
    geo_argused(adj2);
    return new_triangle(i, j, k);
...
/**
 * \brief Copies a Voronoi cell from a Delaunay triangulation.
 * \param[in] delaunay a pointer to the Delaunay triangulation
 * \param[in] i the index of the vertex of which the Voronoi cell
 *  should be computed.
 * \param[out] C the Voronoi cell.
