...
void ConvexCell::init_with_box(double xmin, double ymin, double zmin,
...
index_t ConvexCell::save(std::ostream& out, index_t v_offset,
                         double shrink) const {
  vec3 g;
  if (shrink != 0.0) {
...
void ConvexCell::clip_by_plane(vec4 eqn) {
  ...
  while (t != END_OF_LIST) {
    ...
    if (triangle_is_in_conflict(T, eqn)) {
      set_triangle_flags(t, ushort(conflict_head) | ushort(CONFLICT_MASK));
      conflict_head = t;
...
bool ConvexCell::triangle_is_in_conflict(TriangleWithFlags T,
                                         const vec4& eqn) const {
  double det = 0.0;
  ...
  if (T.i == VERTEX_AT_INFINITY) {
    ...
    det = -det3x3(eqn.x, n2.x, n3.x, eqn.y, n2.y, n3.y, eqn.z, n2.z, n3.z);
  } else if (T.j == VERTEX_AT_INFINITY) {
    vec3 n3 = vertex_plane_normal(T.k);
    vec3 n1 = vertex_plane_normal(T.i);
    det = -det3x3(n1.x, eqn.x, n3.x, n1.y, eqn.y, n3.y, n1.z, eqn.z, n3.z);
  } else if (T.k == VERTEX_AT_INFINITY) {
    vec3 n1 = vertex_plane_normal(T.i);
    vec3 n2 = vertex_plane_normal(T.j);
    det = -det3x3(n1.x, n2.x, eqn.x, n1.y, n2.y, eqn.y, n1.z, n2.z, eqn.z);
  } else {
    ...
    det = det4x4(p1.x, p2.x, p3.x, eqn.x, p1.y, p2.y, p3.y, eqn.y, p1.z, p2.z,
                 p3.z, eqn.z, p1.w, p2.w, p3.w, eqn.w);
  }
  return (det > 0.0);
}
...
vec4 ConvexCell::compute_triangle_point(TriangleWithFlags T) const {
  ...
  if (T.i == VERTEX_AT_INFINITY) {
    ...
    vec3 Njk = normalize(
        cross(make_vec3(Pj.x, Pj.y, Pj.z), make_vec3(Pk.x, Pk.y, Pk.z)));
    ...
    vec3 Nki = normalize(
        cross(make_vec3(Pk.x, Pk.y, Pk.z), make_vec3(Pi.x, Pi.y, Pi.z)));
    ...
    vec4 Pj = vertex_plane(T.j);
    vec3 Nij = normalize(
        cross(make_vec3(Pi.x, Pi.y, Pi.z), make_vec3(Pj.x, Pj.y, Pj.z)));
  ...
  if (v2t_[v] != END_OF_LIST) {
    ...
    do {
      if (cur < 2) {
        ...
        result += triangle_area(triangle_point_[t1t2[0]],
                                triangle_point_[t1t2[1]], triangle_point_[t]);
        t1t2[1] = ushort(t);
      }
      ++cur;
      index_t lv = triangle_find_vertex(t, v);
...
inline double tet_volume(vec3 p1, vec3 p2, vec3 p3, vec3 p4) {
  ...
  return ::fabs(UVx * Wx + UVy * Wy + UVz * Wz) / 6.0;
}
  ...
  for (index_t v = 0; v < nb_v_; ++v) {
    ...
    do {
      if (cur < 2) {
        ...
        result +=
            tet_volume(triangle_point_[t_origin], triangle_point_[t1t2[0]],
                       triangle_point_[t1t2[1]], triangle_point_[t]);
        t1t2[1] = ushort(t);
      }
      ++cur;
...
double ConvexCell::squared_radius(vec3 center) const {
  ...
  while (t != END_OF_LIST) {
    ...
    if (geometry_dirty_) {
      ...
      vec3 p3 = make_vec3(p4.x / p4.w, p4.y / p4.w, p4.z / p4.w);
...
/**
 * \brief Helper function for copy_Laguerre_cell_from_Delaunay()
 */
GEO::index_t copy_Laguerre_cell_facet_from_Delaunay(
    GEO::Delaunay* delaunay, GEO::index_t i, const GEO::vec3& Pi, double wi,
    double Pi_len2, GEO::index_t t, VBW::ConvexCell& C,
    GEO::vector<GEO::index_t>& neighbors) {
  // Local tet vertex indices from facet
  // and vertex in facet indices.
  static GEO::index_t fv[4][3] = {{1, 3, 2}, {0, 2, 3}, {3, 1, 0}, {0, 1, 2}};

  GEO::index_t f = delaunay->index(t, GEO::signed_index_t(i));
  ...
  for (int lfv = 0; lfv < 3; ++lfv) {
    jkl[lfv] = GEO::index_t(delaunay->cell_vertex(t, fv[f][lfv]));

    // Case 1: infinite vertex
    l_jkl[lfv] = (jkl[lfv] == GEO::index_t(VERTEX_AT_INFINITY))
    ...
    if (l_jkl[lfv] == VBW::index_t(VERTEX_NOT_FOUND)) {
      ...
      GEO::vec3 Pj(delaunay->vertex_ptr(jkl[lfv]));
      double wj = 0.0;  // power diagram weight, = 0 for Voronoi.
      double a = 2.0 * (Pi[0] - Pj[0]);
      double b = 2.0 * (Pi[1] - Pj[1]);
      double c = 2.0 * (Pi[2] - Pj[2]);
      double d =
          (Pj[0] * Pj[0] + Pj[1] * Pj[1] + Pj[2] * Pj[2]) - Pi_len2 + wi - wj;
      C.create_vertex(VBW::make_vec4(a, b, c, d));
    }
  }
...
void copy_Voronoi_cell_from_Delaunay(GEO::Delaunay* delaunay, GEO::index_t i,
                                     VBW::ConvexCell& C,
                                     GEO::vector<GEO::index_t>& neighbors) {
  geo_assert(delaunay->stores_cicl());

  C.clear();
  neighbors.resize(0);

  // Create the vertex at infinity.
  C.create_vertex(VBW::make_vec4(0.0, 0.0, 0.0, 0.0));
  GEO::vec3 Pi(delaunay->vertex_ptr(i));
  double wi = 0.0;  // power diagram weight, = 0 for Voronoi
  ...
  GEO::index_t t = GEO::index_t(delaunay->vertex_cell(i));

  // Special case: Laguerre cell is empty (vertex has
  // no incident tet). Will not happen for Voronoi.
  if (t == (GEO::index_t)(-1)) {
    return;
  }
  ...
  do {
    GEO::index_t f = copy_Laguerre_cell_facet_from_Delaunay(
        delaunay, i, Pi, wi, Pi_len2, t, C, neighbors);
    t = GEO::index_t(delaunay->next_around_vertex(t, f));
  } while (t != GEO::index_t(delaunay->vertex_cell(i)));
}
