...
bool is_to_fix_facet_cc(const std::vector<MedialSphere> &all_medial_spheres,
  ...
  // for (auto& pair : msphere.pcell.neigh_seed_to_cells) {
  ...
  //       halfplane_cells, msphere.pcell.cell_neighbors,
  //       msphere.pcell.facet_cc_cells[neigh_id]);
  //   get_facet_CC_surf_v2fids(msphere.pcell.cell_to_surfv2fid,
  //                            msphere.pcell.facet_cc_cells,
  //                            msphere.pcell.facet_cc_surf_v2fids);
  //   printf(
  //       "[FacetCC] msphere %d with neigh_id: %d has halfplane_cells: %ld, "
  //       "num_facet_cc: %d \n",
  //       msphere.id, neigh_id, halfplane_cells.size(), num_facet_cc);
  //   if (num_facet_cc > 1 + SCALAR_ZERO_1) {
  //     msphere.pcell.neigh_seed_is_fixed[neigh_id] = false;  // not fixed yet
  ...
  for (auto &pair : msphere.pcell.facet_cc_cells) {
    // if (pair.second.size() > 1 + SCALAR_ZERO_1) is_debug = true;
    ...
    if (num_facet_cc > 1 + SCALAR_ZERO_1) is_to_fix = true;
    if (!is_to_fix) continue;
    ...
    if (is_to_fix && !msphere.pcell.ce_covered_lvids.empty() &&
      ...
      // printf(") \n");
      // printf("[FacetCC] sphere %d cover concave line: (", msphere_neigh.id);
      for (const auto &code5_neigh : msphere_neigh.pcell.ce_covered_lvids) {
...
void check_cc_and_euler(
  ...
  for (auto &msphere : all_medial_spheres) {
    ...
    if (msphere.pcell.topo_status == Topo_Status::unkown) continue;

    int seed_id = msphere.id;
    const std::set<int> &all_cell_ids = msphere.pcell.cell_ids;
    // assert(all_cell_ids.size() > 0);
    ...
    bool is_to_fix = false;
    // check Cells
    // printf("calling is_to_fix_voro_cc ...\n");
    ...
    //   //
    //   is_to_fix = is_to_fix_facet_euler(convex_cells_host, msphere,
    //   is_debug);
    // }
    // // check Edges
    // if (!is_to_fix) {
 ...
 *
 * There are 3 topo_type to fix for now.
 * 1. For Topo_Status::high_cell_cc, we pick random fid from sf_mesh in
 * untouched powercell.
 * 2. For Topo_Status::low_cell_euler, we pick fid from sf_mesh with maximum
 * distance to sphere center
 * 3. For Topo_Status::high_facet_cc:
 *    3.1 For powercell covers concave edges, we add a new pin on a random
 * selected concave edge, then add a new pin sphere.
 *    3.2 Else, we pick fid from sf_mesh near the halfplane with maximum
 * distance to a random cell on the halfplane. We add ${num_facet_cc-1} new
 * spheres.
 *
 * @param sf_mesh
...
int fix_topo_by_adding_new_sphere(
  ...
  for (int mid : spheres_to_fix) {
    ...
    if (msphere.pcell.topo_status == Topo_Status::high_facet_cc ||
        msphere.pcell.topo_status == Topo_Status::low_facet_euler) {
      const auto &neigh_seed_is_fixed = msphere.pcell.neigh_seed_is_fixed;
      ...
      // fix each halfplane
      for (const auto &pair : neigh_seed_is_fixed) {
        ...
        if (!all_medial_spheres.at(neigh_id).fcc_is_to_fix(msphere.id)) {
          printf(
              "[Fix] halfplane [%d, %d] has one sphere %d not need to fix "
              "FacetCC \n",
              msphere.id, neigh_id, neigh_id);
        ...
        if (pair.second == true) {
          printf("[Fix] halfplane [%d, %d] has been fixed \n", msphere.id,
                 neigh_id);
          continue;
        }
        // Condition3:
        // fix Topo_Status::high_cell_cc first
        if (all_medial_spheres.at(neigh_id).pcell.topo_status ==
            Topo_Status::high_cell_cc) {
          printf("[Fix] neigh_id %d has high CC, fix it first \n", neigh_id);
          continue;
        }
        ...
        if (is_debug)
          printf(
              "[Fix] msphere %d adding new sphere for neigh_id: %d, "
              "num_new_spheres: %d \n",
              msphere.id, neigh_id, num_new_spheres);
        for (int i = 0; i < num_new_spheres; i++) {
          ...
          int tmp_cell_id =
              *std::next(fcc_cells_in_group[i].begin(), tmp_rand_id);
          ...
          FOR(lvid, cc_trans.nb_v) {
            ...
            FOR(i, 3) {
              ...
              is_point_found = true;
              point =
                  to_vec(cc_trans.compute_vertex_coordinates(cmake_uchar3(v)));
              printf(
                  "[Fix] msphere %d found cell %d with lv %d on lf %d, point "
                  "(%f,%f,%f)\n",
                  msphere.id, cc_trans.id, lvid, lf, point[0], point[1],
                  point[2]);
              break;
            }
          ...
          printf(
              "[Fix] msphere %d found v2fid_chosen %d, pin (%f,%f,%f) "
              "sf_mesh facets size: %d, fcc_cells_in_group size: %ld, "
              "fcc_v2fids_in_group size: %ld, random_cell_id: %d \n",
              msphere.id, v2fid_chosen.second, v2fid_chosen.first[0],
      ...
      continue;
    }  // if Topo_Status::high_facet_cc or Topo_Status::low_facet_euler
    ...
    if (msphere.is_on_se() &&
        msphere.pcell.topo_status == Topo_Status::high_cell_cc) {
      // // collect cells that contains SE (to skip)
      // std::set<int> cells_to_skip;
      // for (const aint5 &tmp : msphere.pcell.se_covered_lvids) {
      //   int cell_id = tmp[0];
      //   int se_line_id = tmp[3];
      //   // printf("msphere %d covered se_line_id: %d \n", msphere.id,
      //   // se_line_id);
      //   if (se_line_id == msphere.se_line_id) cells_to_skip.insert(cell_id);
      // }
      // if (is_debug) {
      //   printf("[Fix] msphere %d cells_to_skip: [", msphere.id);
      //   for (const auto &cell_id : cells_to_skip) {
      //     printf("%d, ", cells_to_skip.size());
      //   }
      //   printf("]\n");
      // }
      ...
      // Note: some bfid match sf_mesh, but some are just assigned uniquely in
      // function load_num_adjacent_cells_and_ids() and match nothing
      for (const auto &surf_v2fids : msphere.pcell.cc_surf_v2fids) {
        ...
        // } else if (msphere.pcell.topo_status == Topo_Status::low_cell_euler)
        // { v2fid_chosen =
        //     get_v2fid_max_to_point(sf_mesh, surf_v2fids, msphere.center);
        // }
      }
    ...
    if (is_debug)
      printf(
          "[Fix] msphere %d has msphere.pcell.cc_surf_v2fids: %ld, "
          "num_new_spheres: %d\n",
    ...
    for (int i = 0; i < num_new_spheres; i++) {
      ...
      if (is_debug)
        printf(
            "[Fix] adding %d/%d, msphere %d found v2fid_chosen %d, sf_mesh "
            "facets size:%d\n",
      ...
      printf("[Fix] msphere %d newly added sphere %d is_good: %d\n", msphere.id,
             all_medial_spheres.back().id, is_good);
    }
  }  // for spheres_to_fix
