...
void MshSaver::save_elements(const VectorI& elements,
  ...
  if (m_binary) {
    ...
    for (size_t i = 0; i < m_num_nodes; i++) {
      ...
      if (m_dim == 3) {
        ...
        fout << node_idx << " " << field[i * 2] << " " << field[i * 2 + 1]
             << " " << zero << std::endl;
      }
    }
  }
  ...
  if (m_binary) {
    ...
    for (size_t i = 0; i < m_num_elements; i++) {
      ...
      if (m_dim == 3) {
        ...
        fout << elem_idx << " " << field[i * 2] << " " << field[i * 2 + 1]
             << " " << zero << std::endl;
      }
    }
  }
  ...
  if (m_binary) {
    for (size_t i = 0; i < m_num_elements; i++) {
      ...
      if (m_dim == 3) {
        ...
        Float tensor[9] = {val[0], val[5], val[4], val[5], val[1],
                           val[3], val[4], val[3], val[2]};
        fout.write((char*)tensor, sizeof(Float) * 9);
      } else if (m_dim == 2) {
        const VectorF& val = field.segment(i * 3, 3);
        Float tensor[9] = {val[0], val[2], zero, val[2], val[1],
                           zero,   zero,   zero, zero};
        fout.write((char*)tensor, sizeof(Float) * 9);
      }
    }
    ...
    for (size_t i = 0; i < m_num_elements; i++) {
      ...
      if (m_dim == 3) {
        ...
        fout << elem_idx << " " << val[0] << " " << val[5] << " " << val[4]
             << " " << val[5] << " " << val[1] << " " << val[3] << " " << val[4]
             << " " << val[3] << " " << val[2] << std::endl;
      } else if (m_dim == 2) {
        const VectorF& val = field.segment(i * 3, 3);
        fout << elem_idx << " " << val[0] << " " << val[2] << " " << zero << " "
             << val[2] << " " << val[1] << " " << zero << " " << zero << " "
             << zero << " " << zero << std::endl;
      }
    }
  }
